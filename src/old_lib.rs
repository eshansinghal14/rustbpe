...
struct Span {
    chunk_start: u32,
    chunk_end: u32,
    /// start offset within `chunk_start`
    start: usize,
    /// end offset within `chunk_end`
    end: usize,
}
...
impl Word {
    ...
    fn new_single_chunk(ids: Vec<u32>) -> Self {
        let n = ids.len();
        let spans = (0..n)
            .map(|i| Span::within_chunk(0, i, i + 1))
            .collect::<Vec<_>>();
        Self {
            ids,
            spans,
    ...
    fn new_from_chunks(chunks: &[CompactString]) -> Self {
        ...
        for (chunk_i, chunk) in chunks.iter().enumerate() {
            ...
            for (j, &b) in bytes.iter().enumerate() {
                ids.push(b as u32);
                spans.push(Span::within_chunk(chunk_i as u32, j, j + 1));
            }
        }
    ...
    #[inline]
    fn new_from_chunk_ids(chunk_pool: &[CompactString], chunk_ids: &[u32]) -> Self {
        let mut ids: Vec<u32> = Vec::new();
        let mut spans: Vec<Span> = Vec::new();
        let mut chunk_lens: Vec<usize> = Vec::with_capacity(chunk_ids.len());

        for (chunk_i, &cid) in chunk_ids.iter().enumerate() {
            let chunk = &chunk_pool[cid as usize];
            let bytes = chunk.as_bytes();
            let clen = bytes.len();
            chunk_lens.push(clen);
            for (j, &b) in bytes.iter().enumerate() {
        ...
        self.ids
            ...
            .filter_map(move |(i, w)| {
                ...
                let ok = Span::can_within_merge(sa, sb)
                    || (allow_superchunk
                        && Span::adjacent_full_chunks(sa, sb, &self.chunk_lens));
    ...
    fn merge_pair(&mut self, pair: Pair, new_id: u32, allow_superchunk: bool) -> Vec<(Pair, i32)> {
        ...
        let mut out_spans: Vec<Span> = Vec::with_capacity(n);
        ...
        while i < n {
                        ...
                        )))
            {
                ...
                let merged_span = if Span::can_within_merge(sa, sb) {
                    ...
                    // superchunk merge (only allowed between two full-chunk/full-superchunk spans)
                    Span {
                        chunk_start: sa.chunk_start,
    ...
    words
        ...
        .map(|(i, w)| {
            ...
            if w.ids.len() >= 2 && counts[i] != 0 {
                for (a, b) in w.pairs_mergeable(allow_superchunk) {
                    *local_pc.entry((a, b)).or_default() += counts[i];
                    local_wtu.entry((a, b)).or_default().push(i);
                }
            }
            (local_pc, local_wtu)
        })
...
impl Tokenizer {
    ...
    #[pyo3(signature = (iterator, vocab_size, pattern=None, allow_superchunk=false, max_superchunk_chunks=4096, low_memory=false))]
    #[pyo3(text_signature = "(self, iterator, vocab_size, pattern=None, allow_superchunk=False, max_superchunk_chunks=4096, low_memory=False)")]
    pub fn train_from_iterator(
        ...
        let mut seq_counts_ids: AHashMap<Vec<u32>, i32> = AHashMap::new();
        let mut chunk_intern: AHashMap<CompactString, u32> = AHashMap::new();
        let mut chunk_pool: Vec<CompactString> = Vec::new();

        let superchunk_splitter = Regex::new(SUPER_CHUNK_PATTERN).expect("SUPER_CHUNK_PATTERN must compile");
        ...
        loop {
            ...
            if allow_superchunk && low_memory {
                total_sequences += buf
                    .iter()
                    .map(|s| superchunk_ranges(s, &superchunk_splitter).len() as u64)
                    .sum::<u64>();
                py.detach(|| {
                    for s in &buf {
                        for (a, b) in superchunk_ranges(s, &superchunk_splitter) {
                            let sc = &s[a..b];
                            let mut pieces: Vec<u32> = Vec::new();
                            for mat in pattern.find_iter(sc) {
                                let piece = mat.expect("regex match failed").as_str();
                                let cs = CompactString::from(piece);
                                let id = if let Some(&id) = chunk_intern.get(&cs) {
                                    id
                                } else {
                                    let id = chunk_pool.len() as u32;
                                    chunk_pool.push(cs.clone());
                                    chunk_intern.insert(cs, id);
                                    id
                                };
                                pieces.push(id);
                            }
                            if max_superchunk_chunks == 0 || pieces.len() <= max_superchunk_chunks {
                                *seq_counts_ids.entry(pieces).or_default() += 1;
                            } else {
                                for chunk in pieces.chunks(max_superchunk_chunks) {
                                    *seq_counts_ids.entry(chunk.to_vec()).or_default() += 1;
                                }
                            }
                        }
                    }
                });
            } else if allow_superchunk {
                total_sequences += buf
                    .iter()
                    .map(|s| superchunk_ranges(s, &superchunk_splitter).len() as u64)
                    .sum::<u64>();
                let local: AHashMap<Vec<CompactString>, i32> = py.detach(|| {
                    buf.par_iter()
                        .map(|s| {
                            let mut m: AHashMap<Vec<CompactString>, i32> = AHashMap::new();
                            for (a, b) in superchunk_ranges(s, &superchunk_splitter) {
                                let sc = &s[a..b];
                                let mut pieces: Vec<CompactString> = Vec::new();
                                for mat in pattern.find_iter(sc) {
                                    let piece = mat.expect("regex match failed").as_str();
                                    pieces.push(CompactString::from(piece));
                                }
                                if max_superchunk_chunks == 0 || pieces.len() <= max_superchunk_chunks {
                                    *m.entry(pieces).or_default() += 1;
                                } else {
                                    for chunk in pieces.chunks(max_superchunk_chunks) {
                                        *m.entry(chunk.to_vec()).or_default() += 1;
                                    }
                                }
                            }
                            m
        ...
        let (words, cvec) = if allow_superchunk {
            if low_memory {
                let mut w = Vec::with_capacity(seq_counts_ids.len());
                let mut cv = Vec::with_capacity(seq_counts_ids.len());
                for (seq, c) in seq_counts_ids.into_iter() {
                    w.push(Word::new_from_chunk_ids(&chunk_pool, &seq));
                    cv.push(c);
                }
                (w, cv)
            } else {
                let mut w = Vec::with_capacity(seq_counts.len());
                let mut cv = Vec::with_capacity(seq_counts.len());
                for (seq, c) in seq_counts.into_iter() {
                    w.push(Word::new_from_chunks(&seq));
                    cv.push(c);
                }
                (w, cv)
            }
        } else {
            let mut w = Vec::with_capacity(chunk_counts.len());
            let mut cv = Vec::with_capacity(chunk_counts.len());
            for (chunk, c) in chunk_counts.into_iter() {
                w.push(Word::new_single_chunk(
        ...
        self.train_core_incremental(words, cvec, vocab_size, allow_superchunk);
        Ok(())
    }
...
mod tests {
    ...
    fn test_word_pairs_empty() {
        let word = Word::new_single_chunk(vec![]);
        let pairs: Vec<Pair> = word.pairs_mergeable(false).collect();
    ...
    fn test_word_pairs_single() {
        let word = Word::new_single_chunk(vec![42]);
        let pairs: Vec<Pair> = word.pairs_mergeable(false).collect();
        assert!(pairs.is_empty());
    }
    ...
    fn test_word_merge_pair() {
        ...
        let _deltas = word.merge_pair((1, 2), 99, false);
    ...
    fn test_word_merge_pair_adjacent() {
        ...
        let mut word = Word::new_single_chunk(vec![1, 2, 1, 2, 1, 2]);
        let _deltas = word.merge_pair((1, 2), 99, false);
    ...
    fn test_train_core_incremental() {
        ...
        let words = vec![
            Word::new_single_chunk(vec![97, 98]),  // "ab"
            Word::new_single_chunk(vec![99, 100]), // "cd"
        ];
        let counts = vec![10, 5];

        // Train with vocab_size = 257 (one merge)
        tok.train_core_incremental(words, counts, 257, false);
    ...
    fn test_word_merge_overlapping_pairs() {
        ...
        let _deltas = word.merge_pair((97, 97), 256, false);
    ...
    fn test_word_merge_multiple_occurrences() {
        ...
        let mut word = Word::new_single_chunk(vec![1, 2, 99, 1, 2]);
        let deltas = word.merge_pair((1, 2), 256, false);
    ...
    fn test_superchunk_ranges_basic() {
        ...
        // SUPER_CHUNK_PATTERN splits on ., !, ? and consumes trailing whitespace.
        assert_eq!(parts, vec!["Hi. ", "Bye! ", "Ok?  ", "Last"]);
    }

    #[test]
    fn test_superchunk_merge_requires_full_chunks() {
        // Two chunks: "a" and "b". Cross-chunk pair (97, 98) should be mergeable
        // when allow_superchunk=true because both chunks are already full tokens.
        let chunks = vec![CompactString::from("a"), CompactString::from("b")];
        let mut w = Word::new_from_chunks(&chunks);

        // With allow_superchunk=false, there should be no mergeable cross-chunk pair.
        let pairs_no: Vec<Pair> = w.pairs_mergeable(false).collect();
        assert!(pairs_no.is_empty());

        // With allow_superchunk=true, (a,b) is mergeable across chunk boundary.
        let pairs_yes: Vec<Pair> = w.pairs_mergeable(true).collect();
        assert_eq!(pairs_yes, vec![(97, 98)]);

        // And merge_pair should apply it only when allow_superchunk=true.
        let _deltas = w.merge_pair((97, 98), 256, true);
        assert_eq!(w.ids, vec![256]);
    }
    ...
    fn test_train_multiple_merges() {
        ...
        tok.train_core_incremental(words, counts, 258, false);
    ...
    fn test_count_pairs_parallel_zero_count() {
        ...
        let words = vec![Word::new_single_chunk(vec![1, 2, 3])];
        let counts = vec![0];
