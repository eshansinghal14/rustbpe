...
fn load_phase1_cache(path: &str, vocab_size: u32, chunk_pattern: &str) -> Option<Vec<(Pair, u64)>> {
    ...
    let v2 = if magic == "RUSTBPE_PHASE1_CACHE_V2" {
        ...
        if magic != "RUSTBPE_PHASE1_CACHE_V1" {
            return None;
        }
        false
    };
    ...
    let pat = pat_line
        .strip_prefix("chunk_pattern=")
        .or_else(|| pat_line.strip_prefix("pattern="))?;
    if pat != chunk_pattern {
        return None;
    }
...
fn save_phase1_cache(path: &str, vocab_size: u32, chunk_pattern: &str, phase1_seq: &[(Pair, u64)]) {
    ...
    if writeln!(f, "RUSTBPE_PHASE1_CACHE_V2").is_err() {
        return;
    }
    if writeln!(f, "vocab_size={}", vocab_size).is_err() {
        return;
    }
    if writeln!(f, "chunk_pattern={}", chunk_pattern).is_err() {
        return;
    }
    ...
    for (k, &((a, b), freq)) in phase1_seq.iter().enumerate() {
        ...
        if writeln!(f, "{} {} {} {} {} {}", a, b, freq, ahex, bhex, mhex).is_err() {
            return;
        }
    }
}
...
fn load_phase2_cache(
    ...
    let v2 = if magic == "RUSTBPE_PHASE2_CACHE_V2" {
        ...
        if magic != "RUSTBPE_PHASE2_CACHE_V1" {
            return None;
        }
        false
    };
    ...
    let merge_lines: Vec<&str> = match lines.next() {
        Some(line) if line.starts_with("superchunk_pattern=") => {
            ...
            if sp != superchunk_pattern {
                return None;
            }
            lines.collect()
        }
        Some(first_merge) => {
            if superchunk_pattern != SUPER_CHUNK_PATTERN {
                return None;
            }
...
fn save_phase2_cache(
    ...
    if writeln!(f, "RUSTBPE_PHASE2_CACHE_V2").is_err() {
        return;
    }
    if writeln!(f, "vocab_size={}", vocab_size).is_err() {
        return;
    ...
    if writeln!(f, "phase1_fp={}", phase1_fp).is_err() {
        return;
    }
    if writeln!(f, "superchunk_pattern={}", superchunk_pattern).is_err() {
        return;
    }
    ...
    for (k, &((l, r), freq)) in phase2_seq.iter().enumerate() {
        ...
        let mhex = bytes_to_hex(&token_bytes[mid as usize]);
        if writeln!(f, "{} {} {} {} {} {}", l, r, freq, lhex, rhex, mhex).is_err() {
            return;
        }
    }
}
        ...
        while i < n {
            ...
            if is_match {
                ...
                let merged_span = if Span::can_within_merge(sa, sb) {
                    Span {
                        chunk_start: sa.chunk_start,
                        chunk_end: sa.chunk_end,
                        start: sa.start,
                        end: sb.end,
                    }
                } else {
                    Span {
                        chunk_start: sa.chunk_start,
                        chunk_end: sb.chunk_end,
                        start: 0,
                        end: self.chunk_lens[sb.chunk_end as usize],
                    }
                };
                out.push(new_id);
    ...
    words
        ...
        .reduce(
            ...
            |(mut apc, mut awtu), (pc, wtu)| {
                for (k, v) in pc {
                    *apc.entry(k).or_default() += v;
                }
                for (k, s) in wtu {
                    awtu.entry(k).or_default().extend(s);
                }
                (apc, awtu)
            },
        )
...
fn train_and_record(
    ...
    allow_superchunk: bool,
    id_offset: u32,
    merge_limit: Option<u32>,
) -> Vec<(Pair, u64)> {
    assert!(vocab_size >= 256, "vocab_size must be at least 256");
    let max_by_vocab = vocab_size - 256;
    let num_merges = merge_limit
        .map(|m| m.min(max_by_vocab))
        .unwrap_or(max_by_vocab);
    ...
    log::info!(
        "train_and_record: {} unique pairs in initial heap",
        pair_counts.len()
    );
    ...
    while merges_done < num_merges {
        ...
        let pct = if num_merges > 0 {
            (merges_done * 100) / num_merges
        } else {
            100
        };
    ...
    log::info!(
        "train_and_record: done — {} merges recorded (superchunk={})",
        merge_sequence.len(),
        allow_superchunk
    );
    merge_sequence
...
fn collect_unique_chunks_sorted(
    seq_counts: &AHashMap<Vec<CompactString>, i32>,
) -> Vec<CompactString> {
    let mut set: AHashSet<CompactString> = AHashSet::new();
...
/// Phase 2 words when each GPT4 chunk is already a single token id (no Phase 1 BPE).
fn build_phase2_words_atomic(
    seq_counts: AHashMap<Vec<CompactString>, i32>,
    chunk_to_id: &AHashMap<CompactString, u32>,
) -> (Vec<Word>, Vec<i32>) {
    let total = seq_counts.len();
    log::info!(
        "build_phase2_words_atomic: encoding {} unique superchunk sequences (atomic chunk tokens)",
        total
    );
    ...
    for (chunk_seq, count) in seq_counts {
        ...
        if processed % log_every == 0 || processed == total {
            ...
            log::info!(
                "build_phase2_words_atomic: {}% ({}/{}) — emitted_words={} skipped_seq={} rate={:.0}/s eta={:.0}s",
    ...
    if skipped > 0 {
        log::info!(
            "build_phase2_words_atomic: skipped {} sequences containing a chunk missing from the atomic vocabulary",
            skipped
        );
    }
    log::info!(
        "build_phase2_words_atomic: built {} Phase 2 words in {:.2}s",
        words.len(),
...
fn remap_id(id: u32, id_remap: &AHashMap<u32, u32>) -> u32 {
    if id < 256 {
        ...
        *id_remap.get(&id).unwrap_or_else(|| {
            panic!(
                "Token ID {} referenced before its producing merge was processed. \
             This indicates a frequency-ordering invariant violation.",
                id
            )
        })
    }
...
///
/// `phase2_internal_offset` is the number of Phase 1 merge outputs (or atomic chunk tokens
/// reserved at ids `256..256+offset`) that precede Phase 2 merge outputs in internal numbering
/// (`256 + offset + k` for the k-th Phase 2 merge).
///
...
fn interleave_merge_sequences(
    ...
    while (ptr1 < phase1_seq.len() || ptr2 < phase2_seq.len()) && next_id < vocab_size {
        ...
        let phase2_id_offset: u32 = phase2_internal_offset;
    ...
    log::info!(
        "interleave_merge_sequences: produced {} global merge entries",
        final_merges.len()
    );
    final_merges
}
...
impl Tokenizer {
    ...
    pub fn new() -> Self {
        Self {
            ...
            superchunk_pattern: None,
            compiled_superchunk: None,
            chunk_atom_to_id: None,
            chunk_atom_id_to_bytes: None,
        }
    }
    ...
    #[pyo3(signature = (iterator, vocab_size, chunk_pattern=None, superchunk_pattern=None, allow_superchunk=false, max_superchunk_chunks=4096, tokenizer_dir=None, max_superchunk_merges=None))]
    #[pyo3(
        text_signature = "(self, iterator, vocab_size, chunk_pattern=None, superchunk_pattern=None, allow_superchunk=False, max_superchunk_chunks=4096, tokenizer_dir=None, max_superchunk_merges=None)"
    )]
    pub fn train_from_iterator(
        ...
        max_superchunk_merges: Option<u32>,
    ) -> PyResult<()> {
        let chunk_pattern_str = chunk_pattern.unwrap_or_else(|| GPT4_PATTERN.to_string());
        let superchunk_pattern_str =
            superchunk_pattern.unwrap_or_else(|| SUPER_CHUNK_PATTERN.to_string());
        ...
        let compiled_superchunk = Regex::new(&superchunk_pattern_str).map_err(|e| {
            pyo3::exceptions::PyValueError::new_err(format!(
                "Invalid superchunk_pattern regex: {}",
                e
            ))
        })?;

        self.chunk_atom_to_id = None;
        self.chunk_atom_id_to_bytes = None;
        self.superchunk_pattern = None;
        self.compiled_superchunk = None;
        ...
        if allow_superchunk && max_superchunk_merges.is_none() {
            if let (Some(ref p1), Some(ref p2)) = (&phase1_cache_path, &phase2_cache_path) {
                if let Some(p1_seq) = load_phase1_cache(p1, vocab_size, &chunk_pattern_str) {
                    ...
                    if let Some(p2_seq) = load_phase2_cache(
                        p2,
                        vocab_size,
                        &chunk_pattern_str,
                        &superchunk_pattern_str,
                        fp,
                    ) {
                        log::info!(
        ...
        let ingest_superchunk = allow_superchunk || max_superchunk_merges.is_some();
        let collect_chunk_counts =
            allow_superchunk && cached_phase1.is_none() && max_superchunk_merges.is_none();
        ...
        let superchunk_splitter = compiled_superchunk.clone();

        let buffer_size: usize = 8192;
        let mut buf: Vec<String> = Vec::with_capacity(buffer_size);

        log::info!(
            "train_from_iterator: starting ingestion (vocab_size={}, allow_superchunk={}, max_superchunk_merges={:?}, buffer_size={})",
            vocab_size, allow_superchunk, max_superchunk_merges, buffer_size
        );
            ...
            pyo3::Python::attach(|py| {
                ...
                loop {
                    ...
                    match next_obj {
                        ...
                        None => {
                            return if pyo3::PyErr::occurred(py) {
                        ...
                        buf.par_iter()
                            .map(|s| {
                                ...
                                for (a, b) in superchunk_ranges(s, &sc_splitter) {
                                    ...
                                    for mat in chunk_re.find_iter(sc) {
                                        let cs = CompactString::from(
                                            mat.expect("regex match failed").as_str(),
                                        );
                                    ...
                                    if pieces.len() > 1 {
                                        if max_superchunk_chunks == 0
                                            || pieces.len() <= max_superchunk_chunks
                                        {
                            ...
                            .reduce(
                                ...
                                |(mut ac, mut aseq), (bc, bseq)| {
                                    ...
                                    for (k, v) in bseq {
                                        *aseq.entry(k).or_default() += v;
                                    }
                                    (ac, aseq)
                                },
                            )
                    });
                    for (k, v) in local_chunks {
                        *chunk_counts.entry(k).or_default() += v;
                    }
                    for (k, v) in local_seqs {
                        *seq_counts.entry(k).or_default() += v;
                    }
                } else {
                    ...
                    buf.par_iter()
                        .map(|s| {
                            ...
                            for mat in chunk_re.find_iter(s) {
                                let cs =
                                    CompactString::from(mat.expect("regex match failed").as_str());
        ...
        if let Some(max_sc) = max_superchunk_merges {
            let unique = collect_unique_chunks_sorted(&seq_counts);
            let u = unique.len() as u32;
            if u == 0 {
                self.merges.clear();
                log::info!(
                    "train_from_iterator: superchunk-only mode — no chunks in multi-piece supersequences; merges cleared"
                );
                return Ok(());
            }
            ...
            self.compiled_superchunk = Some(compiled_superchunk);
            log::info!(
                "train_from_iterator: done — {} merge rules; vocab_size {}",
                self.merges.len(),
                self.vocab_size()
            );
            return Ok(());
        ...
        if allow_superchunk {
            ...
            let phase1_seq = if let Some(seq) = cached_phase1.take() {
                ...
                log::info!(
                    "train_from_iterator: Phase 1 — within-chunk BPE over {} unique chunks",
                    chunk_counts.len()
                );
                let (p1_words, p1_counts): (Vec<Word>, Vec<i32>) = chunk_counts
                    .iter()
                    .map(|(chunk, &count)| {
                        (
                            Word::new_single_chunk(
                    ...
                    .unzip();
                let seq = train_and_record(p1_words, p1_counts, vocab_size, false, 0, None);
                log::info!(
                    "train_from_iterator: Phase 1 complete — {} within-chunk merges",
                    seq.len()
                );
                if let Some(ref p) = phase1_cache_path {
            ...
            log::info!(
                "train_from_iterator: Phase 2 — cross-chunk BPE over {} unique sequences",
                seq_counts.len()
            );
            let phase2_seq = if let Some(ref p2) = phase2_cache_path {
                if let Some(seq) = load_phase2_cache(
                    p2,
                    vocab_size,
                    &chunk_pattern_str,
                    &superchunk_pattern_str,
                    ...
                    log::info!(
                        "train_from_iterator: loaded Phase 2 cache from {} ({} merges)",
                        p2,
                        seq.len()
                    );
                    seq
                    ...
                    log::info!(
                        "train_from_iterator: Phase 2 complete — {} cross-chunk merges",
                        seq.len()
                    );
                    save_phase2_cache(
                        p2,
            ...
            let (words, counts): (Vec<Word>, Vec<i32>) = chunk_counts
                .iter()
                .map(|(chunk, &count)| {
                    (
                        Word::new_single_chunk(
                ...
                .unzip();
            let seq = train_and_record(words, counts, vocab_size, false, 0, None);
            self.merges = seq
                .iter()
                .enumerate()
                .map(|(k, &(pair, _))| (pair, 256 + k as u32))
                .collect();
            log::info!(
                "train_from_iterator: done — {} merge rules",
                self.merges.len()
            );
        }
            ...
            for (a, b) in superchunk_ranges(text, sc_re) {
                ...
                for m in self.compiled_chunk_pattern.find_iter(sc) {
                    let chunk = match m {
                        Ok(mat) => mat.as_str(),
                        Err(e) => {
                            log::warn!("encode: regex error, skipping chunk: {}", e);
                            continue;
                        }
                    };
                    pieces.push(chunk);
    ...
    pub fn decode(&self, ids: Vec<u32>) -> PyResult<String> {
        ...
        if let Some(ref chunks) = self.chunk_atom_id_to_bytes {
            for (i, bytes) in chunks.iter().enumerate() {
                ...
                vocab[tid as usize] = bytes.clone();
            }
        }
        let mut sorted: Vec<_> = self.merges.iter().collect();
        sorted.sort_by_key(|&(_, &tid)| tid);
        for (&(left, right), &mid) in &sorted {
            ...
            if vocab.len() <= mid as usize {
                vocab.resize(mid as usize + 1, Vec::new());
            }
...
mod tests {
    ...
    fn test_word_new_single_chunk_empty() {
        assert!(Word::new_single_chunk(vec![])
            .pairs_mergeable(false)
            .collect::<Vec<_>>()
            .is_empty());
    }
    ...
    fn test_word_new_single_chunk_one_element() {
        assert!(Word::new_single_chunk(vec![42])
            .pairs_mergeable(false)
            .collect::<Vec<_>>()
            .is_empty());
    }
    ...
    fn test_word_new_phase2_no_within_pairs() {
        ...
        assert!(
            word.pairs_mergeable(false).collect::<Vec<_>>().is_empty(),
    ...
    fn test_train_and_record_selects_highest_freq_first() {
        ...
        let seq = train_and_record(words, vec![10, 5], 257, false, 0, None);
        assert_eq!(seq.len(), 1);
    ...
    fn test_train_and_record_chained_merges() {
        ...
        let seq = train_and_record(words, vec![10], 258, false, 0, None);
        assert_eq!(seq.len(), 2);
    ...
    fn test_train_and_record_no_pairs_returns_empty() {
        ...
        let seq = train_and_record(words, vec![10], 257, false, 0, None);
        assert!(seq.is_empty());
    }
    ...
    fn test_train_and_record_freq_ordering() {
        ...
        let seq = train_and_record(words, vec![100, 50], 258, false, 0, None);
    ...
    fn test_train_and_record_phase2_cross_chunk_only() {
        ...
        let seq = train_and_record(words, vec![5], 257, true, 0, None);
        assert_eq!(seq.len(), 1);
        assert_eq!(seq[0].0, (97, 98));
    }
    ...
    fn test_build_phase2_words_no_within_pairs() {
        ...
        assert!(words[0]
            ...
            .is_empty());
        assert_eq!(
            words[0].pairs_mergeable(true).collect::<Vec<_>>(),
            vec![(97, 98)]
        );
    }
    ...
    fn test_two_phase_pipeline_within_before_cross() {
        ...
        let p1_seq = train_and_record(p1_words, vec![100], 257, false, 0, None);
        assert_eq!(p1_seq[0].0, (97, 98));

        let mut seq_counts: AHashMap<Vec<CompactString>, i32> = AHashMap::new();
        seq_counts.insert(
            vec![CompactString::from("ab"), CompactString::from("c")],
            80,
        );
        let (p2_words, p2_counts) = build_phase2_words(seq_counts, &p1_seq);
        let p2_seq = train_and_record(p2_words, p2_counts, 257, true, p1_seq.len() as u32, None);
    ...
    #[test]
    fn test_superchunk_only_atomic_multibyte_chunks() {
        let mut seq_counts: AHashMap<Vec<CompactString>, i32> = AHashMap::new();
        seq_counts.insert(
            vec![CompactString::from("hello"), CompactString::from("world")],
            10,
        );
        let unique = collect_unique_chunks_sorted(&seq_counts);
        assert_eq!(
    ...
    fn test_encode_decode_roundtrip() {
        ...
        let tok = Tokenizer {
            merges,
            chunk_pattern: r"\w+|\s+".to_string(),
            compiled_chunk_pattern: Regex::new(r"\w+|\s+").unwrap(),
            ..Default::default()
        };
        let text = "hi there";
    ...
    fn test_get_mergeable_ranks_with_merge() {
        ...
        let tok = Tokenizer {
            merges,
            chunk_pattern: String::new(),
            compiled_chunk_pattern: Regex::new("").unwrap(),
            ..Default::default()
        };
        let ranks = tok.get_mergeable_ranks();
